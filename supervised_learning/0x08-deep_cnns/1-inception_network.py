...
def inception_network():
    ...
    conv_1 = K.layers.Conv2D(filters=64, kernel_size=7,
                             ...
                             strides=2, kernel_initializer=init)(inputs)

    max_pool_1 = K.layers.MaxPooling2D(pool_size=[3, 3],
                                       strides=2, padding='same')(conv_1)

    conv_2 = K.layers.Conv2D(filters=64, kernel_size=1, padding='same',
                             activation='relu', strides=1,
                             kernel_initializer=init)(max_pool_1)
    conv_3 = K.layers.Conv2D(filters=192, kernel_size=3,
                             padding='same', activation='relu',
                             strides=1, kernel_initializer=init)(conv_2)

    max_pool_2 = K.layers.MaxPooling2D(pool_size=[3, 3], strides=2,
                                       padding='same')(conv_3)

    conc_1 = inception_block(max_pool_2, [64, 96, 128, 16, 32, 32])
    conc_2 = inception_block(conc_1, [128, 128, 192, 32, 96, 64])

    max_pool_3 = K.layers.MaxPooling2D(pool_size=[3, 3], strides=2,
                                       padding='same')(conc_2)
    ...
    max_pool_4 = K.layers.MaxPooling2D(pool_size=[3, 3], strides=2,
                                       padding='same')(conc_7)
    ...
    avg_pool = K.layers.AveragePooling2D(pool_size=[7, 7], strides=1,
                                         padding='valid',
                                         data_format=None)(conc_9)

    drop = K.layers.Dropout(1 - 0.2)(avg_pool)
